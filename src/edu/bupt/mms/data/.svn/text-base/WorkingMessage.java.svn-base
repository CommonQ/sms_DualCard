...
import edu.bupt.mms.model.TextModel;
import edu.bupt.mms.transaction.MessageSender;
import edu.bupt.mms.transaction.MmsMessageSender;
import edu.bupt.mms.transaction.SmsMessageSender;
import edu.bupt.mms.ui.ComposeMessageActivity;
import edu.bupt.mms.ui.MessageUtils;
import edu.bupt.mms.ui.SlideshowEditor;
import edu.bupt.mms.util.DraftCache;
import edu.bupt.mms.util.Recycler;
import edu.bupt.mms.util.ThumbnailManager;
import edu.bupt.mms.widget.MmsWidgetProvider;
...
public class WorkingMessage {
	...
	private static final String[] MMS_OUTBOX_PROJECTION = { Mms._ID, // 0
			Mms.MESSAGE_SIZE // 1
	};
	...
	public interface MessageStatusListener {
		 ...
		 * 
		 * @param mms
		 *            If true, it changed to MMS. If false, to SMS.
		 */
	...
	public static WorkingMessage loadDraft(ComposeMessageActivity activity,
			final Conversation conv, final Runnable onDraftLoaded) {
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE))
			LogTag.debug("loadDraft %s", conv);
		...
		new AsyncTask<Void, Void, Pair<String, String>>() {
			...
			protected Pair<String, String> doInBackground(Void... none) {
				...
				if (TextUtils.isEmpty(draftText)) {
					...
					Uri uri;
					try {
						uri = readDraftMmsMessage(msg.mActivity, conv, sb);
					} catch (Exception e) {
						uri = null;
						e.printStackTrace();
					...
					if (uri != null) {
						if (msg.loadFromUri(uri)) {
							...
							// will put the subject in our supplied
							// StringBuilder.
							subject = sb.toString();
						}
					}
	...
	public void removeAttachment(boolean notify) {
		...
		if (notify) {
			// Tell ComposeMessageActivity (or other listener) that the
			// attachment has changed.
			// In the case of ComposeMessageActivity, it will remove its
			// attachment panel because
	...
	public static void removeThumbnailsFromCache(SlideshowModel slideshow) {
		if (slideshow != null) {
			ThumbnailManager thumbnailManager = MmsApp.getApplication()
					.getThumbnailManager();
			...
			while (iterator.hasNext()) {
				...
				if (slideModel.hasImage()) {
					thumbnailManager.removeThumbnail(slideModel.getImage()
							.getUri());
					removedSomething = true;
				} else if (slideModel.hasVideo()) {
					thumbnailManager.removeThumbnail(slideModel.getVideo()
							.getUri());
					removedSomething = true;
				}
			}
			if (removedSomething) {
				// HACK: the keys to the thumbnail cache are the part uris, such
				// as mms/part/3
				// Because the part table doesn't have auto-increment ids, the
				// part ids are reused
				// when a message or thread is deleted. For now, we're clearing
				// the whole thumbnail
				// cache so we don't retrieve stale images when part ids are
				// reused. This will be
	...
	public int setAttachment(int type, Uri dataUri, boolean append) {
		...
		int result = OK;
		SlideshowEditor slideShowEditor = new SlideshowEditor(mActivity,
				mSlideshow);

		// Special case for deleting a slideshow. When ComposeMessageActivity
		// gets told to
		// remove an attachment (search for
		// AttachmentEditor.MSG_REMOVE_ATTACHMENT), it calls
		// this function setAttachment with a type of TEXT and a null uri.
		// Basically, it's turning
		// the working message from an MMS back to a simple SMS. The various
		// attachment types
		// use slide[0] as a special case. The call to ensureSlideshow below
		// makes sure there's
		// a slide zero. In the case of an already attached slideshow,
		// ensureSlideshow will do
		// nothing and the slideshow will remain such that if a user adds a
		// slideshow again, they'll
		// see their old slideshow they previously deleted. Here we really
		// delete the slideshow.
		if (type == TEXT && mAttachmentType == SLIDESHOW && mSlideshow != null
				&& dataUri == null && !append) {
			slideShowEditor.removeAllSlides();
		}

		// Make sure mSlideshow is set up and has a slide.
		ensureSlideshow(); // mSlideshow can be null before this call, won't be
							// afterwards
		...
		if (type == IMAGE) {
			// Prime the image's cache; helps A LOT when the image is coming
			// from the network
		...
		mStatusListener.onAttachmentChanged(); // have to call whether succeeded
												// or failed,
												// because a replace that fails,
												// removes the slide
		...
		if (!append && mAttachmentType == TEXT && type == TEXT) {
			...
			/*
			 * SmsMessage.calculateLength returns an int[4] with: int[0] being
			 * the number of SMS's required, int[1] the number of code units
			 * used, int[2] is the number of code units remaining until the next
			 * message. int[3] is the encoding type that should be used for the
			 * message.
			 */
			...
			if (!MmsConfig.getMultipartSmsEnabled()) {
				...
				setLengthRequiresMms(threshold > 0
						&& smsSegmentCount > threshold, false);
			}
		} else {
	...
	/**
	 * Returns true if FORCE_MMS is set. When saveAsMms() has been called, we
	 * set FORCE_MMS to represent sort of an "invisible attachment" so that the
	 * message isn't thrown away when we are shipping it off to other
	 * activities.
	 */
	...
	/**
	 * Change the message's attachment to the data in the specified Uri. Used
	 * only for single-slide ("attachment mode") messages. If the attachment
	 * fails to attach, restore the slide to its original state.
	 */
	private int changeMedia(int type, Uri uri, SlideshowEditor slideShowEditor) {
		...
		// Clear the attachment type since we removed all the attachments. If
		// this isn't cleared
		// and the slide.add fails (for instance, a selected video could be too
		// big), we'll be
		// left in a state where we think we have an attachment, but it's been
		// removed from the
		// slide.
		mAttachmentType = TEXT;
		...
		if (result != OK) {
			...
			if (originalSlide != null) {
				slideShowEditor.addSlide(0, originalSlide); // restore the
															// original slide.
			}
		}
		return result;
	...
	/**
	 * Add the message's attachment to the data in the specified Uri to a new
	 * slide.
	 */
	private int appendMedia(int type, Uri uri, SlideshowEditor slideShowEditor) {
		...
		// The first time this method is called, mSlideshow.size() is going to
		// be
		// one (a newly initialized slideshow has one empty slide). The first
		// time we
		// attach the picture/video to that first empty slide. From then on when
		// this
		// function is called, we've got to create a new slide and add the
		// picture/video
		// to that new slide.
		...
		if (result != OK) {
			// We added a new slide and what we attempted to insert on the slide
			// failed.
			// Delete that slide, otherwise we could end up with a bunch of
			// blank slides.
			// It's ok that we're removing the slide even if we didn't add it
			// (because it was
			// the first default slide). If adding the first slide fails, we
			// want to remove it.
	...
	/**
	 * Returns true if the message has a real slideshow, as opposed to just one
	 * image attachment, for example.
	 */
	...
	/**
	 * Sets the MMS subject of the message. Passing null indicates that there is
	 * no subject. Passing "" will result in an empty subject being added to the
	 * message, possibly triggering a conversion to MMS. This extra bit of state
	 * is needed to support ComposeMessageActivity converting to MMS when the
	 * user adds a subject. An empty subject will be removed before saving to
	 * disk or sending, however.
	 */
	...
	public void forceMms(boolean isTrue) {
		updateState(FORCE_MMS, isTrue, isTrue);
	}
	...
	/**
	 * Returns true if this message has an MMS subject. A subject has to be more
	...
	/**
	 * Moves the message text into the slideshow. Should be called any time the
	 * message is about to be sent or written to disk.
	 */
	private void syncTextToSlideshow() {
		...
		if (!slide.hasText()) {
			...
			text = new TextModel(mActivity, ContentType.TEXT_PLAIN,
					"text_0.txt", mSlideshow.getLayout().getTextRegion());
			slide.add(text);
		} else {
	...
	/**
	 * Sets the message text out of the slideshow. Should be called any time a
	 * slideshow is loaded from disk.
	 */
	...
	/**
	 * Gets internal message state ready for storage. Should be called any time
	...
	public void syncWorkingRecipients() {
		if (mWorkingRecipients != null) {
			ContactList recipients = ContactList.getByNumbers(
					mWorkingRecipients, false);
			mConversation.setRecipients(recipients); // resets the threadId to
														// zero
	...
	public String getWorkingRecipients() {
		...
		ContactList recipients = ContactList.getByNumbers(mWorkingRecipients,
				false);
		return recipients.serialize();
	}

	// Call when we've returned from adding an attachment. We're no longer
	// forcing the message
	// into a Mms message. At this point we either have the goods to make the
	// message a Mms
	...
	public Uri saveAsMms(boolean notify) {
		...
		if (mDiscarded) {
			LogTag.warn("saveAsMms mDiscarded: true mConversation: "
					+ mConversation + " returning NULL uri and bailing");
			return null;
		}
		...
		try {
			...
			// If we don't already have a Uri lying around, make a new one. If
			// we do
			// have one already, make sure it is synced to disk.
			if (mMessageUri == null) {
				mMessageUri = createDraftMmsMessage(persister, sendReq,
						mSlideshow, null);
			} else {
				updateDraftMmsMessage(mMessageUri, persister, mSlideshow,
						sendReq);
			}
			mHasMmsDraft = true;
		} finally {
	...
	public void saveDraft(final boolean isStopping) {
		// If we have discarded the message, just bail out.
		if (mDiscarded) {
			LogTag.warn("saveDraft mDiscarded: true mConversation: "
					+ mConversation + " skipping saving draft and bailing");
			return;
		}
		...
		if (mConversation == null) {
			throw new IllegalStateException(
					"saveDraft() called with no conversation");
		}
		...
		// Get ready to write to disk. But don't notify message status when
		// saving draft
		...
		if (requiresMms()) {
			...
			// we didn't nullify conv.mThreadId, causing a temperary situation
			// where conv
			...
			if (!TextUtils.isEmpty(content)) {
				...
				// When there's no associated text message, we have to handle
				// the case where there
				// might have been a previous mms draft for this message. This
				// can happen when a
				// user turns an mms back into a sms, such as creating an mms
				// draft with a picture,
	...
	/**
	 * To be called from our Activity's onSaveInstanceState() to give us a
	 * chance to stow our state away for later retrieval.
	 * 
	 * @param bundle
	...
	/**
	 * To be called from our Activity's onCreate() if the activity manager has
	 * given it a Bundle to reinflate
	 * 
	 * @param bundle
	...
	/**
	 * Update the temporary list of recipients, used when setting up a new
	 * conversation. Will be converted to a ContactList on any save event (send,
	 * save draft, etc.)
	 */
	public void setWorkingRecipients(List<String> numbers) {
		Log.v(TAG, "setWorkingRecipients");
	...
	public void setConversation(Conversation conv) {
		if (DEBUG)
			LogTag.debug("setConversation %s -> %s", mConversation, conv);

		mConversation = conv;

		// Convert to MMS if there are any email addresses in the recipient
		// list.
	...
	/**
	 * Hint whether or not this message will be delivered to an an email
	 * address.
	 */
	...
	/**
	 * Set whether or not we want to send this message via MMS in order to avoid
	 * sending an excessive number of concatenated SMS messages.
	 * 
	...
	private void updateState(int state, boolean on, boolean notify) {
		if (!sMmsEnabled) {
			// If Mms isn't enabled, the rest of the Messaging UI should not be
			// using any
			// feature that would cause us to to turn on any Mms flag and show
			// the
		...
		if (oldState != mMmsState) {
			if (Log.isLoggable(LogTag.APP, Log.VERBOSE))
				LogTag.debug("updateState: %s%s = %s", on ? "+" : "-",
	 ...
	 *             if sending an MMS and uaProfUrl is not defined in
	 *             mms_config.xml.
	 */
	public void send(final String recipientsInUI) {
		...
		// setText("����һ��vcard��ʽ�ĸ���Ӵ��");

		// Log.v(TAG, "msgTxt: "+msgTxt+" mAttachmentType"+mAttachmentType);
		...
		if (requiresMms() || addressContainsEmailToMms(conv, msgTxt)) {
			...
			Log.v(TAG, "mText: " + mText + " mAttachmentType" + mAttachmentType
					+ "mMessageUri: " + mMessageUri);
	...
	private void preSendSmsWorker(Conversation conv, String msgText,
		...
		if (LogTag.SEVERE_WARNING
				&& ((origThreadId != 0 && origThreadId != threadId) || (!semiSepRecipients
						.equals(recipientsInUI) && !TextUtils
						.isEmpty(recipientsInUI)))) {
		...
		// just do a regular send. We're already on a non-ui thread so no need
		// to fire
	...
	private void sendSmsWorker(String msgText, String semiSepRecipients,
		...
		if (LogTag.VERBOSE || Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
			Log.d(LogTag.TRANSACTION,
					"sendSmsWorker sending message: recipients="
		...
		try {
			...
			Recycler.getSmsRecycler().deleteOldMessagesByThreadId(mActivity,
					threadId);
		} catch (Exception e) {
	...
	private void sendMmsWorker(Conversation conv, Uri mmsUri,
		...
		try {
			...
			if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
				LogTag.debug("sendMmsWorker: update draft MMS message "
						+ mmsUri + " threadId: " + threadId);
			}
			...
			String[] dests = conv.getRecipients().getNumbers(true /*
																 ...
																 */);
			if (dests.length == 1) {
				// verify the single address matches what's in the database. If
				// we get a different
			...
			if (newMessage) {
				// Write something in the database so the new message will
				// appear as sending
			...
			// If user tries to send the message, it's a signal the inputted
			// text is
			// what they wanted.
			UserHappinessSignals.userAcceptedImeText(mActivity);

			// First make sure we don't have too many outstanding unsent
			// message.
		...
		if (MultiSimConfig.isMultiSimEnabled()) {
			...
			values.put(Mms.SUB_ID, MSimTelephonyManager.getDefault()
					.getPreferredDataSubscription());
		}
		SqliteWrapper.update(mActivity, mContentResolver, mmsUri, values, null,
				null);
	...
	// 010copy
//	private static Uri readDraftMmsMessage(Context context, String threadId,
//			StringBuilder sb) {
//		if (DEBUG)
//			Log.e("readDraftMmsMessage tid=%d", threadId);
//		Cursor cursor;
//		ContentResolver cr = context.getContentResolver();
...
//	}
	...
	/**
	 * makeSendReq should always return a non-null SendReq, whether the dest
	 * addresses are valid or not.
	 */
	...
	private static Uri createDraftMmsMessage(PduPersister persister,
			SendReq sendReq, SlideshowModel slideshow, Uri preUri) {
		if (slideshow == null) {
			return null;
		}
		try {
			...
			sendReq.setBody(pb);
			Uri res = persister.persist(sendReq,
					preUri == null ? Mms.Draft.CONTENT_URI : preUri);
			slideshow.sync(pb);
			return res;
		} catch (MmsException e) {
			return null;
	...
	private void asyncUpdateDraftMmsMessage(final Conversation conv,
		...
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			LogTag.debug("asyncUpdateDraftMmsMessage conv=%s mMessageUri=%s",
					conv, mMessageUri);
		}
			...
			public void run() {
				try {
					...
					if (mMessageUri == null) {
						...
						updateDraftMmsMessage(mMessageUri, persister,
								mSlideshow, sendReq);
					}
					if (isStopping && conv.getMessageCount() == 0) {
						// createDraftMmsMessage can create the new thread in
						// the threads table (the
						// call to createDraftMmsDraftMessage calls
						// PduPersister.persist() which
						// can call Threads.getOrCreateThreadId()). Meanwhile,
						// when the user goes
						// back to ConversationList while we're saving a draft
						// from CMA's.onStop,
						// ConversationList will delete all threads from the
						// thread table that
						// don't have associated sms or pdu entries. In case our
						// thread got deleted,
						// well call clearThreadId() so ensureThreadId will
						// query the db for the new
						// thread.
						conv.clearThreadId(); // force us to get the updated
												// thread id
					}
	...
	private static final String SMS_DRAFT_WHERE = Sms.TYPE + "="
	...
	private String readDraftSmsMessage(Conversation conv) {
		...
		Uri thread_uri = ContentUris.withAppendedId(
				Sms.Conversations.CONTENT_URI, thread_id);
		String body = "";

		Cursor c = SqliteWrapper.query(mActivity, mContentResolver, thread_uri,
				SMS_BODY_PROJECTION, SMS_DRAFT_WHERE, null, null);
		...
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			LogTag.debug("readDraftSmsMessage haveDraft: ",
	...
	private void asyncUpdateDraftSmsMessage(final Conversation conv,
...
//学着网上乱改的
	...
	private void deleteDraftSmsMessage(long threadId) {
		SqliteWrapper.delete(mActivity, mContentResolver, ContentUris
				.withAppendedId(Sms.Conversations.CONTENT_URI, threadId),
