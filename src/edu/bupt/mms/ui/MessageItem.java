...
import edu.bupt.mms.ui.MessageListAdapter.ColumnsMap;
...
import edu.bupt.mms.util.PduLoaderManager;
...
public class MessageItem {
    ...
    String mTimestamp;
    String mDatestamp;//010
    String mNewTimestamp;//010
    String mAddress;
    String mContact;
...
//                mNewTimestamp = MessageUtils.justTimeStampString(context, date);//for failed time divider
...
//            if (!isOutgoingMessage()) {
			if (!isOutgoingMessageExceptFailed()) {
				...
				if(date==0){
					...
					service_center = cursor.getString(cursor.getColumnIndexOrThrow("body"));
					//service_center_address
					int index=-1;
					if(service_center!=null&&!service_center.equals("")){
					index=service_center.indexOf("***");
					}
					String sub_center="";
					if(index!=-1){
					
						sub_center = service_center.substring(service_center.indexOf("***")+3);
						date=Long.valueOf(sub_center);
					}else{
						Time now = new Time();
						now.setToNow();
						date=now.toMillis(false);
					}
					
							
					
					Log.v("zzz", "service_center - " + service_center+" sub_center: "+sub_center);
				...
				Log.v("zzz", "mTimestamp - " + mTimestamp);
				mDatestamp = MessageUtils.dateTimeStampString(context, date);
				// 010获取信息的日期：年月日
				Log.v("zzz", "mDatestamp - " + mDatestamp);
				mNewTimestamp = MessageUtils.justTimeStampString(context, date);
				// 010获取信息的时间：只包含日期之后的具体时间
				Log.v("zzz", "mNewTimestamp - " + mNewTimestamp);
			}
            ...
            mTimestamp = null;
            mDatestamp = null;//010
//            long date = cursor.getLong(columnsMap.mColumnMmsDate);
//            mNewTimestamp = MessageUtils.justTimeStampString(context, date);
            mNewTimestamp = null;//010
	...
	public boolean isOutgoingMessageExceptFailed() {
		boolean isOutgoingMms = isMms() && (mBoxId == Mms.MESSAGE_BOX_OUTBOX);
		boolean isOutgoingSms = isSms()
    ...
    public class PduLoadedMessageItemCallback implements ItemLoadedCallback {
        public void onItemLoaded(Object result, Throwable exception) {
            ...
            long timestamp = 0L;//010 attention
            //010added
            long datestamp = 0L;
...
//            010end
            ...
            if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                ...
                datestamp = notifInd.getExpiry() * 1000L;//010
                newtimestamp = notifInd.getExpiry() * 1000L;//010
            } else {
                if (mCursor.isClosed()) {
                    return;
                }
                ...
                if (mMessageType == PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF) {
                    if (msg == null) {
                        ...
                        datestamp = retrieveConf.getDate() * 1000L;//010
                        newtimestamp = retrieveConf.getDate() * 1000L;//010
                    }
                } else {
                    ...
                    datestamp = msg == null ? 0 : ((SendReq) msg).getDate() * 1000L;//010
                    newtimestamp = msg == null ? 0 : ((SendReq) msg).getDate() * 1000L;//010
                }
            ...
            if (!isOutgoingMessage()) {
//                if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
//                    mDatestamp = mContext.getString(R.string.expire_on,
//                            MessageUtils.dateTimeStampString(mContext, datestamp));
//                } else {
                	mDatestamp =  MessageUtils.dateTimeStampString(mContext, datestamp);
//                }
            }
            //010time
            if (!isOutgoingMessage()) {
//                if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
//                    mNewTimestamp = mContext.getString(R.string.expire_on,
//                            MessageUtils.justTimeStampString(mContext, newtimestamp));
//                } else {
                	mNewTimestamp =  MessageUtils.justTimeStampString(mContext, newtimestamp);
//                }
            }
            //010
