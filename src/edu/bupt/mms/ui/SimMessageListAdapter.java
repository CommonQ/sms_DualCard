 ...
 */
public class SimMessageListAdapter extends CursorAdapter {
	...
	public SimMessageListAdapter(Context context, Cursor c, ListView listView,
	...
	public void bindView(View view, Context context, Cursor cursor) {
		if (view instanceof MessageListItem) {
			...
			Log.v("MessageListAdapter", "status: " + status + " type: " + type+" date:"+date
					+ " msgId: " + msgId);

			MessageItem msgItem = getCachedMessageItem(type, msgId, cursor);
			if (msgItem != null) {
				
				msgItem.mMessageType=12306;
	...
	public interface OnDataSetChangedListener {
		void onDataSetChanged(SimMessageListAdapter adapter);

		void onContentChanged(SimMessageListAdapter adapter);
	}
	...
	public void cancelBackgroundLoading() {
		mMessageItemCache.evictAll(); // causes entryRemoved to be called for
										// each MessageItem
										// in the cache which causes us to
										// cancel loading of
	...
	public View newView(Context context, Cursor cursor, ViewGroup parent) {
		...
		// qqq
		Log.v("MessageListAdapter", "boxType: " + boxType);

		View view = mInflater
				.inflate(
						(boxType == INCOMING_ITEM_TYPE_SMS || boxType == INCOMING_ITEM_TYPE_MMS) ? R.layout.message_list_item_recv
								: R.layout.message_list_item_send, parent,
						false);
		if (boxType == INCOMING_ITEM_TYPE_MMS
				|| boxType == OUTGOING_ITEM_TYPE_MMS) {
			// We've got an mms item, pre-inflate the mms portion of the view
			view.findViewById(R.id.mms_layout_view_stub).setVisibility(
					View.VISIBLE);
		}
		return view;
	}
		...
		if (item == null && c != null && isCursorValid(c)) {
			try {
				item = new MessageItem(mContext, type, c, mColumnsMap,
						mHighlight);
				Log.v("MessageListAdapter", "item.mType: " + item.mType
						+ "item.mMsgId: " + item.mMsgId);
