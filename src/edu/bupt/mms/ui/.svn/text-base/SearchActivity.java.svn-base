...
public class SearchActivity extends ListActivity {
	...
	/*
	 * Subclass of TextView which displays a snippet of text which matches the
	 * full text and highlights the matches within the snippet.
	 */
	public static class TextViewSnippet extends TextView {
		 ...
		 * Do that here and then defer to super for whatever work is normally
		 * done.
		 */
		@Override
		protected void onLayout(boolean changed, int left, int top, int right,
				int bottom) {
			//Log.e("onLayout", mFullText);
			if (mFullText == null) {
				mFullText = "";
			}
			...
			textFieldWidth -= (2F * ellipsisWidth); // assume we'll need one on
													// both ends
	...
	private long getThreadId(long sourceId, long which) {
		Uri.Builder b = Uri.parse("content://mms-sms/messageIdToThread")
				.buildUpon();
		...
		Cursor c = getContentResolver().query(Uri.parse(s), null, null, null,
				null);
		if (c != null) {
			try {
				if (c.moveToFirst()) {
	...
	public void onCreate(Bundle icicle) {
		...
		String searchStringParameter = getIntent().getStringExtra(
				SearchManager.QUERY);
		if (searchStringParameter == null) {
			searchStringParameter = getIntent().getStringExtra(
					"intent_extra_data_key" /*
											 * SearchManager.
											 * SUGGEST_COLUMN_INTENT_EXTRA_DATA
											 */);
		}
		final String searchString = searchStringParameter != null ? searchStringParameter
				.trim() : searchStringParameter;

		// If we're being launched with a source_id then just go to that
		// particular thread.
		// Work around the fact that suggestions can only launch the search
		// activity, not some
		...
		if (u != null && u.getQueryParameter("source_id") != null) {
			Thread t = new Thread(new Runnable() {
				public void run() {
					try {
						long sourceId = Long.parseLong(u
								.getQueryParameter("source_id"));
						long whichTable = Long.parseLong(u
								.getQueryParameter("which_table"));
		...
		setContentView(R.layout.search_activity);//搜索结果
		tx=(TextView)findViewById(android.R.id.empty);//010
		...
		// When the query completes cons up a new adapter and set our list
		// adapter to that.
		mQueryHandler = new AsyncQueryHandler(cr) {
			protected void onQueryComplete(int token, Object cookie, Cursor c) {
				if (c == null) {
					tx.setText("No matches!!");
					return;
				}
				...
				setTitle(getResources().getQuantityString(
						R.plurals.search_results_title, cursorCount,
						cursorCount, searchString));

				
				if(cursorCount<1)
					tx.setText(getResources().getString(R.string.search_empty));//无搜索结果010
				else{
				...
				setListAdapter(new CursorAdapter(SearchActivity.this, c, false /*
					...
					public void bindView(View view, Context context,
						...
						final TextViewSnippet snippet = (TextViewSnippet) (view
								.findViewById(R.id.subtitle));

						String address = cursor.getString(addressPos);
						Contact contact = address != null ? Contact.get(
								address, false) : null;

						String titleString = contact != null ? contact
								.getNameAndNumber() : "";
						title.setText(titleString);

						snippet.setText(cursor.getString(bodyPos), searchString);

						// if the user touches the item then launch the compose
						// message
						// activity with some extra parameters to highlight the
						// search
						// results and scroll to the latest part of the
						// conversation
						// that has a match.
					...
					public View newView(Context context, Cursor cursor,
						...
						View v = inflater.inflate(R.layout.search_item, parent,
								false);
						return v;
					}
				...
				// ListView seems to want to reject the setFocusable until such
				// time
				// as the list is not empty. Set it here and request focus.
				// Without
				// this the arrow keys (and trackball) fail to move the
				// selection.
				listView.setFocusable(true);
				...
				if (cursorCount > 0) {
					...
					if (recent != null) {
						recent.saveRecentQuery(
								searchString,
								getString(R.string.search_history, cursorCount,
										searchString));
					}
				}
			}
