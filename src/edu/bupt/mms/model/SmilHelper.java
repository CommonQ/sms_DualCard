...
import static edu.bupt.mms.dom.smil.SmilMediaElementImpl.SMIL_MEDIA_END_EVENT;
import static edu.bupt.mms.dom.smil.SmilMediaElementImpl.SMIL_MEDIA_PAUSE_EVENT;
import static edu.bupt.mms.dom.smil.SmilMediaElementImpl.SMIL_MEDIA_SEEK_EVENT;
import static edu.bupt.mms.dom.smil.SmilMediaElementImpl.SMIL_MEDIA_START_EVENT;
import static edu.bupt.mms.dom.smil.SmilParElementImpl.SMIL_SLIDE_END_EVENT;
import static edu.bupt.mms.dom.smil.SmilParElementImpl.SMIL_SLIDE_START_EVENT;

import edu.bupt.mms.MmsApp;
import edu.bupt.mms.dom.smil.SmilDocumentImpl;
import edu.bupt.mms.dom.smil.parser.SmilXmlParser;
    ...
    private static SMILDocument createSmilDocument(PduBody pb) {
        ...
        for (int i = 0; i < partsNum; i++) {
            ...
            if (contentType.equals(ContentType.TEXT_PLAIN)
                ...
                hasText = true;
                //zaizhe
            }else if (contentType.equalsIgnoreCase(ContentType.TEXT_VCARD)) {
    ...
    private static SMILDocument createSmilDocument(SlideshowModel slideshow) {
        ...
        for (SlideModel slide : slideshow) {
            ...
            for (MediaModel media : slide) {
                ...
                //zaizhe 我我也要修改！！！！    qq_vcard 这里负责的是我发出去的是以vcard模式显示还是以文本方式显示。所以我需要的是解决去掉tag发送的短信我接收时需要正确的识别用vcard封装。
                else if (media instanceof VCardModel) {
                
                	sme = SmilHelper.createMediaElement(SmilHelper.ELEMENT_TAG_VCARD, document, src);
