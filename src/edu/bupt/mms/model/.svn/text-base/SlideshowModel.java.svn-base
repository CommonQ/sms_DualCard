...
import edu.bupt.mms.dom.smil.parser.SmilXmlSerializer;
import edu.bupt.mms.layout.LayoutManager;
...
public class SlideshowModel extends Model
    ...
    public static SlideshowModel createFromPduBody(Context context, PduBody pb) throws MmsException {
        ...
        for(int i=0;i<pb.getPartsNum();i++){
        	...
        	PduPart pp =pb.getPart(i);
        	String str3 = new String(pp.getContentType());
        
        	Log.v(TAG, "pp.getContentType(): "+str3);
        }
        ...
        for (int i = 0; i < regionsNum; i++) {
            ...
            Log.v(TAG, "regionsNum: "+regionsNum);
            regions.add(r);
        }
        ...
        for (int i = 0; i < slidesNum; i++) {
            ...
            Log.v(TAG, "mediaNum: "+mediaNum);
            ArrayList<MediaModel> mediaSet = new ArrayList<MediaModel>(mediaNum);

            //所以需要手动将vcardmodel加入mediaset中！！ qq 
            
            if(mediaNum==0||mediaNum==1){
            	
            	
            	for(int j=0;j<pb.getPartsNum();j++){
            	PduPart pp =pb.getPart(j);
            	String str3 = new String(pp.getContentType());
            	
            	if(str3.equalsIgnoreCase("text/x-vcard")){
            		
            		Log.v(TAG, "text/x-vcard: "+j);
            		
            		String src=getPartSrc(pp);
            		
            		 byte[] bytes = pp.getContentType();
            		 String contentType = new String(bytes);
            	//	 VCardModel	media=createNewVcardModel(context,contentType,src,pp.getDataUri());
            		 VCardModel	media = new VCardModel(context, contentType, src,
            				pp.getDataUri());
            		 media.setBegin(0);
...
//            		String name = null,name2 = null,name3 = null,name4 = null;
//            		if(pp.getFilename()!=null)
//            		 name = new String(pp.getFilename());
//            		if(pp.getName()!=null)
//            		 name2 = new String(pp.getName());
//            		if(pp.getContentLocation()!=null)
//            		 name3 = new String(pp.getContentLocation());
//            		if(pp.getContentId()!=null)
//            		 name4 = new String(pp.getContentId());
//            		Log.v(TAG, "name:"+name+"name2"+name2+"name3: "+name3+"name4: "+name4);
                    ...
                    if (!MmsConfig.getSlideDurationEnabled()) {
                        ...
                        if ((int)mediadur / 1000 != dur) {
                            ...
                            if (ContentType.isVideoType(media.mContentType)
                              ...
                              ||media.mContentType.equalsIgnoreCase("text/x-vcard")
                              || tag.equals(SmilHelper.ELEMENT_TAG_AUDIO)||ContentType.isTextType(media.mContentType)||tag.equals(SmilHelper.ELEMENT_TAG_VCARD)) {
                                /*
    ...
    private static String getPartSrc(PduPart pp) {
    	...
    	if (pp.getFilename() != null)
            str = new String(pp.getFilename());
          else if (pp.getName() != null)
            str = new String(pp.getName());
          else
            str = "unknown";
		return str;
	}
